...
def split_stereo(input_path: str, tmpdir: str) -> tuple[str, str]:
    ...
    if audio.channels != 2:
        print(f"⚠️  チャンネル数が {audio.channels} です。ステレオ(2ch)を想定しています。")
        if audio.channels == 1:
...
def main():
    ...
    parser.add_argument("--left-name", default="Speaker_L",
                        help="左チャンネルの話者名 (default: Speaker_L)")
    parser.add_argument("--right-name", default="Speaker_R",
                        help="右チャンネルの話者名 (default: Speaker_R)")
    parser.add_argument("--output", default=None,
                        help="出力ファイルパス (default: {input}_transcript.{format})")
    parser.add_argument("--format", default="tsv", choices=["txt", "tsv", "srt", "json"],
                        help="出力形式 (default: tsv)")
    ...
    formatters = {"txt": output_txt, "tsv": output_tsv, "srt": output_srt, "json": output_json}
    ...
    print("\n✨ 完了！")
    print("   次のステップ: 出力ファイルをClaudeにアップロードして編集・ショーノート作成")
